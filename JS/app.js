...
monthlyBtn.addEventListener('click', () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateNotifications = () => {
      ...
      // Show up to 5 notifications
      const numNotifications = Math.min(notifications.length, 5);
      for (let i = 0; i < numNotifications; i++) {
        const notificationElement = createNotificationElement(notifications[i]);
        notificationsDropdown.appendChild(notificationElement);
      }
      // Update notification dot and alert banner
      notificationDot.textContent = numNotifications.toString();
      notificationDot.style.display = numNotifications > 0 ? 'block' : 'none';
      const notificationCount = Math.min(notifications.length, 5);
...
// When the 'send' button is clicked, the `sendMessage` function is called.
...
updateNotifications();
// Calls the `updateNotifications` function to update the notifications in the user interface.
});